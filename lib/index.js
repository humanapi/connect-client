...
function mountOnClickListener() {
    ...
    const hapiElements = Array.from(document.querySelectorAll("[data-hapi-token]").values());
...
function load(options) {
    ...
    logger(`Load with options: ${JSON.stringify(options)}`);
...
function on(eventName, eventListener) {
    if (typeof eventListeners[eventName] !== "object") {
...
function emit(eventName, ...args) {
    if (typeof eventListeners[eventName] === "object") {
        eventListeners[eventName].forEach(listener => listener.apply(this, args));
    }
}
