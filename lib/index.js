...
const VIEWPORT_CONTENT = "width=device-width, initial-scale=1, maximum-scale=1";

const eventListeners = {};
let focusTrap = null;
let wasVierpotrMetaAdded = false;
let defaultViewportScale = null;
let defaultInnerWidth = null;
...
function load(options = {}) {
    ...
    function getViewportMetaTag() {
        ...
        for (let i = 0; i < metas.length; i++) {
            if (metas[i].getAttribute("name") === "viewport" && metas[i].getAttribute("content") === VIEWPORT_CONTENT) {
                logger("Viewport meta tag exist");
                return metas[i];
            }
        }
        return;
    ...
    function verifyMetaViewport() {
        // calculate default viewport scale in case we need to reset it after closing the modal
        const viewportWidth = Math.max(document.documentElement.clientWidth, window.innerWidth || 0);
        const screenWidth = window.screen.width;
        defaultViewportScale = screenWidth / viewportWidth;
        // save current innerWidth in case we need to reset the viewport on cleanup
        defaultInnerWidth = window.innerWidth;
        logger("Verifying viewport meta tag");
        // Verify that the viewport tag exists before inserting the iframe
        // so the modal looks good in mobile browsers:
        // <meta content="width=device-width, initial-scale=1, maximum-scale=1" name="viewport">
        if (!getViewportMetaTag()) {
            logger("Creating viewport meta tag");
            const viewport = document.createElement("meta");
            viewport.setAttribute("name", "viewport");
            viewport.content = VIEWPORT_CONTENT;
            document.getElementsByTagName("head")[0].appendChild(viewport);
            wasVierpotrMetaAdded = true;
        }
    }
    ...
    function cleanupElements() {
        ...
        if (wasVierpotrMetaAdded) {
            getViewportMetaTag().remove();
            const viewport = document.createElement("meta");
            viewport.setAttribute("name", "viewport");
            viewport.setAttribute("content", `width=${defaultInnerWidth}, initial-scale=${defaultViewportScale}`);
            document.getElementsByTagName("head")[0].appendChild(viewport);
            wasVierpotrMetaAdded = false;
        }
    }
