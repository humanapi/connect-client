...
function load(options) {
    ...
    const tokenElement = document.querySelector('[data-hapi-token]');
    let sessionToken;
    if (!tokenElement) {
        console.warn("Passing the session token as a parameter for the HumanConnect.open() has been deprecated. Please use the data attribute 'data-hapi-token' instead. See http://www.example.com for details");
        if (!options.token) throw new Error("The token attribute is required");
        sessionToken = options.token;
    } else {
        sessionToken = tokenElement.dataset.hapiToken;
    }
...
/*function trackCtaLoad(token, referrer){
    const xhr = new XMLHttpRequest();
    const cookiesRaw = document.cookie.split(";");
    const uidCookie = cookiesRaw.filter(cookie => cookie.startsWith("hapi-uid="))
        .map(cookie => cookie.split("=")[1]);

    if (uidCookie && uidCookie.length === 0) {
        //xhr.addEventListener("load", reqListener);
        xhr.open("GET", `http://analytics.humanapi.co/api/v1/analytics/ctaLoaded?referrer=${referrer}&cid=${token}`);
        xhr.send();
    }
    ...
    xhr.onload = function(progressEvent) {
        if (xhr.status === 200) {
            ...
            document.cookie = `hapi-uid=${uuid}`;
        }
    };
}*/

/*window.addEventListener("load", (event) => {
    const sessionToken = document.querySelectorAll('[data-hapi-token]');
